package tools.implicits

import akka.cluster.Member
import queue.QueuesManager
...
package object members {
  implicit final class BetterMember(member: Member) {
    def actorSelection(path: String)(implicit system: ActorSystem = QueuesManager.system()): ActorSelection = {
      QueuesManager.system().actorSelection(ActorPath.fromString(member.address.toString) / path)
    }
    def userActorSelection(path: String)(implicit system: ActorSystem = QueuesManager.system()): ActorSelection = {
      system.actorSelection(ActorPath.fromString(member.address.toString) / "user" / path)
    }
  }
}
